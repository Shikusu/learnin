  ...
  try {
    ...
    try {
      ...
      if (
        ...
        !Array.isArray(parsed.key_concepts) || parsed.key_concepts.length === 0 ||
        !parsed.outline || typeof parsed.outline !== "object" ||
        !Array.isArray(parsed.mnemonics)
      ) {
        return NextResponse.json(
          { error: "Gemini returned incomplete data. Try again." },
          { status: 500 }
        );
      }
    } catch {
    ...
    console.error("[generate-fiche]", err);
    if (err.status === 429) {
      return NextResponse.json(
